...
void print_dpx_header(dpx_image_t *dpx, int endian) {
    dcp_log(LOG_DEBUG,"%-15.15s: magic num %d","read_dpx",r_32(dpx->file.magic_num,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: offset %d","read_dpx",r_32(dpx->file.offset,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: version %s","read_dpx",dpx->file.version);
    dcp_log(LOG_DEBUG,"%-15.15s: file size %d","read_dpx",r_32(dpx->file.file_size,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: file name %s","read_dpx",dpx->file.file_name);
    dcp_log(LOG_DEBUG,"%-15.15s: creator %s","read_dpx",dpx->file.creator);

    dcp_log(LOG_DEBUG,"%-15.15s: orientation %d","read_dpx",r_16(dpx->image.orientation,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: elem number %d","read_dpx",r_16(dpx->image.element_number,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: pixels_per_line %d","read_dpx",r_32(dpx->image.pixels_per_line,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: line per elem %d","read_dpx",r_32(dpx->image.lines_per_image_ele,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: descriptor %d","read_dpx",dpx->image.image_element[0].descriptor);
    dcp_log(LOG_DEBUG,"%-15.15s: transfer %d","read_dpx",dpx->image.image_element[0].transfer);
    dcp_log(LOG_DEBUG,"%-15.15s: colorimetric %d","read_dpx",dpx->image.image_element[0].colorimetric);
    dcp_log(LOG_DEBUG,"%-15.15s: bit size: %d","read_dpx",dpx->image.image_element[0].bit_size);
    dcp_log(LOG_DEBUG,"%-15.15s: packing: %d","read_dpx",r_16(dpx->image.image_element[0].packing,endian));
    dcp_log(LOG_DEBUG,"%-15.15s: encoding: %d","read_dpx",r_16(dpx->image.image_element[0].encoding,endian));
...
int read_dpx(odcp_image_t **image_ptr, int dpx_log, const char *infile, int fd) {
    ...
    dcp_log(LOG_DEBUG,"%-15.15s: opening dpx file %s","read_dpx",infile);
    if (fd == 0) {
    ...
    /* create the image */
    dcp_log(LOG_DEBUG,"%-15.15s: allocating odcp image","read_dpx");
    image = odcp_image_create(3,w,h);
    dcp_log(LOG_DEBUG,"%-15.15s: image allocated","read_dpx");
