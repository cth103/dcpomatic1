...
def configure(conf):
    ...
    waflib.Logs.pprint('YELLOW', '')
    if conf.env.TARGET_WINDOWS:
        waflib.Logs.pprint('YELLOW', '\t' + 'Target'.ljust(25) + ': Windows')
    elif conf.env.TARGET_LINUX:
        waflib.Logs.pprint('YELLOW', '\t' + 'Target'.ljust(25) + ': Linux')
    elif conf.env.TARGET_OSX:
        waflib.Logs.pprint('YELLOW', '\t' + 'Target'.ljust(25) + ': OS X')
    ...
    def report(name, variable):
        ...
        if variable:
            ...
            linkage = 'dynamic'
        waflib.Logs.pprint('YELLOW', '\t%s: %s' % (name.ljust(25), linkage))
